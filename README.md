...
Pre-built installers for every tagged release are attached to the [GitHub Releases page](https://github.com/BenjaminRose805/jobcopilot/releases):

| Platform | File |
| --- | --- |
| Windows | `JobCopilot-Setup-*-win-x64.exe` (installer) or `JobCopilot-*-win-x64.zip` (portable) |
| macOS — Apple Silicon | `JobCopilot-*-mac-arm64.dmg` |
| macOS — Intel | `JobCopilot-*-mac-x64.dmg` |
...
Builds are produced by [`.github/workflows/release.yml`](.github/workflows/release.yml) on every `v*.*.*` tag push, running the full `verify` gate on Windows, macOS (arm64 + x64) and Linux runners in parallel.
